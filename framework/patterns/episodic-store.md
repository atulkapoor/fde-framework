...
realizations:
  ...
  - {stack: supermemory, template: memory/episodic-store.supermemory.py.j2, provides: Store}
...
The supermemory realization is offered where the profile already runs the
engine: extraction, contradiction resolution and forgetting become its job,
and the hosted endpoint is refused behind a boundary.
