...
def _ci(architecture, out: Path) -> None:
    ...
    workflows.joinpath("ci.yml").write_text(
        ...
        "      # The deliverable's own tests: the contract, the fence, the\n"
        "      # harness's refusal of an empty exam, and every promise the\n"
        "      # HTTP edge makes, asserted against the booted service --\n"
        "      # model-free, seconds.\n"
        "      - name: Smoke and edge\n"
